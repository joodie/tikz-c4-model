...
\begin{tikzpicture}[node distance=3cm]
  ...
  \draw node[c4 system] (ibs) {
    \c4text{Internet Banking System} {Software System} {
  ...
  \draw node[c4 person, above=of ibs] (user) {
    \c4text{Internet Banking User}{Person} {
      Customers will view...
  ...
  \draw node[c4 existing system, right=of ibs] (es) {
    \c4text{E-Mail System} {Software System} {
  ...
  \draw node[c4 existing system, below=of ibs] (mbs) {
    \c4text{Mainframe Banking System} {Software System} {
  ...
  \draw node[c4 database, right=of mbs] (db) {
    \c4text{Database} {SQL} {
      Data Store handling data and
      transactions for Banking System
    }
  };
  ...
  \draw [->, draw=c4existingsystemcolor!80!black, thick] (mbs.east) to [ edge
  label=Uses] (db.west);
